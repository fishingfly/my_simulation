...
void ChannelAccess::initialize( int stage )
{
    ...
    if( stage == 0 ){
        ...
        //add in the 4-27 by xpl to add the gate signal
        GateConnectionNum =nic->registerSignal("GateConnectionNum");
        GateDirectConnectionNum =nic->registerSignal("GateDirectConnectionNum");
        SendToChannelNum =nic->registerSignal("SendToChannelNum");
    }
    ...
    if(useSendDirect){
        ...
        if( i != gateList.end() ){
            ...
            for(; i != --gateList.end(); ++i){
                ...
                {
                    //add in the 4-27 to emit gate signal
                    //if(simTime()>30 && simTime()<100)
                    //{
                    emit(GateDirectConnectionNum,1);
                    //}
