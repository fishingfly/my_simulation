...
void Mac1609_4::handleSelfMsg(cMessage* msg) {
	if (msg == nextChannelSwitch) {
		...
		if(pktToSend->getIsBroadcast())//zy
        {
            mac->setDestAddr(LAddress::L2BROADCAST());
        }
        else
        {
            mac->setDestAddr(pktToSend->getMacAddr());
        }
...
void Mac1609_4::handleLowerMsg(cMessage* msg) {
    ...
    //add by zhouyu
    long source=macPkt->getSrcAddr();
    wsm->setMacAddr(source);
    //end add
	...
	else if (dest == LAddress::L2BROADCAST()) {//change by zhouyu
